...
(function($) {
	...
	mesour._editable.fields.Number = function(fieldStructure, editable, element, parameters, identifier) {
		...
		var _this = this,
			...
			input = text.getInput(),
			popover = text.getEditablePopover();

		function fixNumber(value) {
			var fixedValue = $.trim(value.replace(new RegExp(separator === '.' ? '\\.' : separator, 'g'), '')
				.replace(decimalPoint, '.')
				.replace(unit, ''));
			if (!mesour._editable.isNumeric(fixedValue)) {
				return '';
			}
			return fixedValue;
		};
		...
		function save() {
			var isValid = mesour._editable.Validators.validateNumber(
				editable,
				input.val(),
				input,
				true,
